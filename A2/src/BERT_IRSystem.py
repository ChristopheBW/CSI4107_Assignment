...
class IRSystem:
    ...
    def calculate_bert_similarity_score(self):
        ...
        # run 50 query takes too much time, select only query from '1' to '5' for testing
        self.queries = {k: v for k, v in self.queries.items() if k in ['1', '2', '3', '4', '5']}

        # Prepare a file to write results in trec_eval format
        with open("trec_eval.txt", "w") as f:
            # Calculate the similarity score for each document
            for query_id, query in tqdm(self.queries.items(), desc="Processing queries"):
                # Tokenize and encode the query
                query_ids = tokenizer.encode(query, add_special_tokens=True, return_tensors='pt', max_length=max_sequence_length, truncation=True).to(device)

                # Get the embeddings of the query
                with torch.no_grad():
                    query_embeddings = model(query_ids)[0]

                # Average the embeddings of tokens in the query
                query_embeddings = torch.mean(query_embeddings, dim=1)

                # Calculate the similarity score for each document
                for docno, doc in tqdm(self.collection.items(), desc="Processing documents", leave=False):
                    # Tokenize and encode the document
                    doc_ids = tokenizer.encode(doc, add_special_tokens=True, return_tensors='pt', max_length=max_sequence_length, truncation=True).to(device)

                    # Get the embeddings of the document
                    with torch.no_grad():
                        doc_embeddings = model(doc_ids)[0]

                    # Average the embeddings of tokens in the document
                    doc_embeddings = torch.mean(doc_embeddings, dim=1)

                    # Calculate the similarity score
                    score = nn.functional.cosine_similarity(query_embeddings, doc_embeddings, dim=-1).item()

                    # Write the result to the file in trec_eval format
                    f.write(f"{query_id}\tQ0\t{docno}\t1\t{score}\tExp\n")
    ...
    def calculate_result(self):
        ...
        for i in range(1, self.Q + 1):
            process_list.append(
                pool.apply_async(self.calculate_bert_similarity_score,
                                (self.queries[str(i)], )))
        pool.close()
        pool.join()
...
if __name__ == '__main__':
    ...
    print("Calculating...")
    irs.calculate_bert_similarity_score()
    
    # load similarity scores from file
    # similarity_scores = {}
    # with open("similarity_scores.txt", "r") as f:
    #     for line in f:
    #         queryno, docno, similarity = line.split()
    #         if queryno not in similarity_scores:
    #             similarity_scores[queryno] = {}
    #         similarity_scores[queryno][docno] = float(similarity)
    ...
    # rank = 1
    # with open('results.txt', 'w') as f:
    #     for queryno, scores in similarity_scores.items():
    #         rank = 1
    #         for docno, similarity in scores.items():
    #             result = f"{queryno} Q0 {docno} {rank} {similarity} bert\n"
    #             f.write(result)
    #             rank += 1


    # print(irs.get_cosine_similarity(irs.queries["1"]))
